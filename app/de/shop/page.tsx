...
const GENRES = [
  ...
  "Dance","Hörspiel","Jazz","Klassik","Country","Italo Disco","Disco","EDM","Hip Hop",
] as const;
...
function buildQuery(next: Record<string, string | undefined>, cur: URLSearchParams) {
  ...
  Object.entries(next).forEach(([k, v]) => (v == null || v === "") ? s.delete(k) : s.set(k, v));
  return `?${s.toString()}`;
}
...
export default async function ShopPage({ searchParams }: { searchParams?: { q?: string; genre?: string; cat?: string } }) {
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
        <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-4 md:gap-6 items-center">
          <div className="p-6 md:p-8">
            ...
            <p className="mt-2 text-white/80">Gebrauchte Vinyls! 12&quot; Maxi Singles, Compilations, Alben, Maxi CDs und div. Specials zum fairen Preis.</p>
            ...
            <form className="mt-5 flex gap-2" action="">
              <input type="search" name="q" placeholder="Suchen (Artist, Titel, EAN, Katalog …)" defaultValue={q} className="flex-1 rounded-lg px-3 py-2 bg-white/5 border border-white/10" />
